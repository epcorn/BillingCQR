...
import HistorySection from "../components/HistorySection"; // Import the clean component
...
const res = await axios.get(
  ...
  const handleUploadClick = (serviceId) => {
    setCurrentTargetServiceId(serviceId);
...
const res = await axios.get(
    ...
    if (isImage)
      return (
        <a href={url} target="_blank" rel="noopener noreferrer">
          <img src={url} style={styles.image} alt="Card" />
        </a>
      );
    if (isDoc)
      return (
        ...
        >
          <button style={styles.viewBtn}>View File</button>
        </a>
      );
    return (
      <a href={url} target="_blank" rel="noopener noreferrer">
        <button style={styles.viewBtn}>Open File</button>
      </a>
    );
  };
  ...
  const handleDownloadList = () => {
    if (!sortedRows.length) return alert("No data available to download!");

    // Prepare Excel data — exactly like labels on the page
    const data = sortedRows.map((row, idx) => ({
      "S.No": idx + 1,
      "Contract / Card Label": row.serviceCardLabel,
    }));
    ...
    saveAs(blob, `Contract_List_${activeTab}_${month}_${year}.xlsx`);
  };
    ...
    <div style={styles.wrapper}>
      ...
      <div style={styles.headerContainer}>
        ...
        <div style={{ display: "flex", gap: "15px" }}>
          <button style={styles.downloadBtn} onClick={handleDownloadList}>Download List</button>
      ...
      <div style={styles.tabBar}>
        <button
          ...
          style={{
            ...styles.tabBtn,
            backgroundColor: activeTab === "due" ? "#4a90e2" : "#e0e0e0",
            color: activeTab === "due" ? "#fff" : "#333",
          }}
        >
          Due Cards
        </button>
        <button
          onClick={() => setActiveTab("afterJob")}
          style={{
            ...styles.tabBtn,
            backgroundColor: activeTab === "afterJob" ? "#4a90e2" : "#e0e0e0",
            color: activeTab === "afterJob" ? "#fff" : "#333",
          }}
        >
          Bill After Job
        </button>
      ...
      <div style={styles.tableWrapper}>
        <table style={styles.table}>
          <thead>
            <tr>
              <th style={styles.thSortable} onClick={handleSort}>
                Contract No / Card # {sortDirection === "asc" ? "↑" : "↓"}
              </th>
          ...
          <tbody>
            {isLoading ? (
              ...
              sortedRows.map(row => {
                ...
                return (
                  <tr key={row.serviceId} style={styles.tableRow}>
                    ...
                    <td style={styles.td}>
                      <div style={{ fontWeight: 600 }}>{row.billTo}</div>
                      {row.billToAddress1 && <div>{row.billToAddress1}</div>}
                      {row.billToAddress2 && <div>{row.billToAddress2}</div>}
                    </td>
                    <td style={styles.td}>
                      <div style={{ fontWeight: 600 }}>{row.shipTo}</div>
                      {row.shipToAddress1 && <div>{row.shipToAddress1}</div>}
                      {row.shipToAddress2 && <div>{row.shipToAddress2}</div>}
                    </td>
                    ...
                    <td style={styles.td}>
                      ...
                      {status === "error" && (
                        <button onClick={() => handleUploadClick(row.serviceId)} style={{ ...styles.uploadBtn, backgroundColor: "#e74c3c" }}>
                          Retry
                        </button>
                      )}
                      {!status && (
                        <button onClick={() => handleUploadClick(row.serviceId)} style={styles.uploadBtn}>
                          Upload
                        </button>
                      )}
                    </td>
              ...
              <tr>
                <td colSpan="6" style={styles.noData}>
                  {activeTab === "afterJob" ? "No after-job cards found." : "No due cards found for selected month and year."}
                </td>
              </tr>
            )}
          </tbody>
...
const styles = {
  ...
  downloadBtn: { background: "#2980b9", color: "#fff", border: "none", padding: "12px 25px", borderRadius: "8px", cursor: "pointer", fontSize: "16px", fontWeight: "600" },
